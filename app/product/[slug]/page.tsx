...
import { getProduct, products as seed, mainImage } from "@/lib/products";
  ...
  return (
    ...
    <main className="mx-auto grid max-w-6xl gap-10 px-4 py-10 md:grid-cols-2">
      <div className="flex flex-col gap-3">
        {p.images.length === 0 && (
          <div className="relative aspect-[3/4] overflow-hidden rounded-2xl border border-white/10">
            <Image src={mainImage(p)} alt={`${p.name} — no photo yet`} fill sizes="50vw" className="object-cover" />
          </div>
        )}
        {p.images.map((src, i) => (
      ...
      <div className="md:sticky md:top-20 md:self-start">
        ...
        <p className="mt-2 text-2xl font-bold"><Price usdAmount={p.priceUsd} compareAt={p.compareAt} /></p>
        ...
        <AddToCart slug={p.slug} name={p.name} price={p.priceUsd} image={mainImage(p)} sizes={p.sizes} />
        ...
        <div className="mt-6 rounded-xl border border-white/10 p-4 text-sm opacity-80">
          <p className="font-bold">{t("shipRetT")}</p>
          <p className="mt-2">{t("shipRetD")}</p>
        </div>
      </div>
    </main>
