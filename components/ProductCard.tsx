    ...
    >
      <button onClick={() => onQuickView(p)} className="block w-full cursor-pointer text-left" aria-label={`Quick view ${p.name}`}>
        <div className="relative aspect-[3/4] overflow-hidden">
          <Image src={p.images[0]} alt={p.name} fill sizes="(max-width:768px) 50vw, 25vw" className="object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
          {p.badge && (
      ...
      <div className="flex items-center justify-between gap-2 p-3">
        ...
        <p className="text-sm font-bold"><Price usdAmount={p.priceUsd} /></p>
      </div>
    </motion.article>
  );
}
