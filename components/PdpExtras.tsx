...
export function PdpExtras({ p, others }: { p: Product; others: Product[] }) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4">
      ...
      <div className="grid gap-6 py-10 md:grid-cols-2">
        <div>
          ...
          {p.video ? (
            ...
            <p className="mt-3 rounded-2xl border border-white/10 p-6 text-sm opacity-60">Video coming for this piece — see the tee spin on the homepage.</p>
          )}
        </div>
        <div>
          ...
          {p.type === "stock" ? (
            <div className="mt-3 rounded-2xl border border-white/10 p-5">
              <p className="font-bold">Only {p.stockQty} left in Indonesia</p>
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10">
                <div className="h-full rounded-full bg-[#C1121F]" style={{ width: `${Math.min(100, Math.round(((p.stockQty ?? 0) / 40) * 100))}%` }} />
              </div>
              <p className="mt-2 text-sm opacity-60">Ships in 24h · Express 3–7 days worldwide.</p>
            </div>
          ) : (
            <div className="mt-3 rounded-2xl border border-white/10 p-5">
              <p className="font-bold">Made to order — never out of stock</p>
              <p className="mt-2 text-sm opacity-60">Printed at the nearest hub (US/EU/Asia) in 3–5 days, then shipped locally.</p>
            </div>
          )}
          <div className="mt-3 rounded-2xl border border-white/10 p-5 text-sm">
            <p className="font-bold">{r.avg} {stars(r.avg)} <span className="font-normal opacity-60">· {r.count} verified reviews</span></p>
      ...
      {/* REVIEWS */}
      <Reveal>
        <h2 className="font-display text-2xl">WORN & RATED</h2>
      ...
      {/* UGC STRIP */}
      <div className="py-10">
        <h2 className="font-display text-2xl">WORN WORLDWIDE</h2>
        ...
        <p className="mt-2 text-xs opacity-50">Tag @aprivateviolence to get featured. Sample strip — real UGC after launch.</p>
      </div>

      {/* CROSS-SELL */}
      <div className="pb-16">
        <h2 className="font-display text-2xl">COMPLETE THE LOOK</h2>
